...
class Config(object):
    ...
    def __init__(self):
        ...
        self.LANGUAGE = 'en'
        # set sample collection
        if sys.platform == 'win32':
            basepath = self.basepath = join(sys.prefix, 'lib','site-packages','pyisis')
            self.OUTPUT_ENCODING = 'cp850'
        else:
            basepath = self.basepath = join(split(os.__file__)[0],'site-packages','pyisis')
            self.OUTPUT_ENCODING = 'utf-8'

        fullpath = join(basepath, 'sample')
        self.COLLECTIONS.append(('IsisCollection', 'sample', [fullpath]))
